...
mhelp = {
    ...
    'spotify module': 'Spotify: now, repeat, derepeat, next, back, pausetr, unpausetr, restr, liketr, auth, codeauth, unauth'
}
...
sp_auth = spotipy.oauth2.SpotifyOAuth(client_id=client_id, client_secret=client_secret, redirect_uri='https://fuccsoc.com/', scope=scope)
...
async def check_token():
    ...
    if db.get('acs_tkn') != None:
        if db.get('LastChange') != None:
            ttc = datetime.datetime.strptime(db.get('LastChange'), "%Y-%m-%dT%H:%M:%S.%f") + datetime.timedelta(minutes=45)
...
async def auth(client: Client, message: Message):
    ...
    if db.get('acs_tkn')!=None:
        ...
        await message.edit(f'<a href="{sp_auth.get_authorize_url()}">Перейдите по этой ссылке</a>, подтвердите доступ, затем скопируйте адрес редиректа и выполните <code>.codeauth [адрес редедиректа]</code>')
...
async def codeauth(client: Client, message: Message):    
    try:
        ...
        await message.edit('⚠️Произошла какая-то ошибка. Проверьте, что вы все делаете верно.')
...
async def now(client: Client, message: Message):    
    ...
    if db.get('spotify_module', 'acs_tkn') == None:
        ...
        try:
            device = current_playback["device"]["name"] + " " + current_playback["device"]["type"].lower()
        except:
        ...
        try:
            ...
            volume = '-1%'
        try:
            percentage = ceil(current_playback["progress_ms"]/current_playback["item"]["duration_ms"]*100)
            bar = ''
        ...
        try:
            ...
            try:
                playlist_owner = playlist["owner"]["display_name"] + ' <code>(' + playlist["owner"]["id"]+ ')</code>'
            except:
